//Visita cada pagina que contiene los resultados de la busqueda hecha en RENATI y las guarda en un archivo consolidado

const fs = require('fs');
const puppeteer = require('puppeteer');

// Definir las rutas de los archivos como variables globales
const INPUT_FILE = './RawURLS/RENATI - SL - {offshore}.txt';
const OUTPUT_FILE = 'C:\\D_DevBox\\TESIS_MBA\\ScrapRepos\\Items_urlList\\RENATI-ItemUrlList-{offshore}.txt';
...
(async () => {
    ...
    for (let url of urls) {
        ...
        const links = await page.$$eval('a[href^="/handle/sunedu/"]', as => as.map(a => a.href.replace('/Save', '')));
